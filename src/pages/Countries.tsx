...
interface Country { id: number; code: string; name: string; dial_code: string; flag: string; enabled: boolean }

export default function Countries() {
  const [list, setList] = useState<Country[]>([]);
  const [draft, setDraft] = useState({ code: "", name: "", dial_code: "+", flag: "🏳️", enabled: true });
  ...
  const create = async () => {
    ...
    try { await api.countries.create(draft); setDraft({ code: "", name: "", dial_code: "+", flag: "🏳️", enabled: true }); load(); toast.success("Country added"); }
  ...
  return (
    <>
      ...
      <div className="glass-card mb-6 p-5">
        <div className="grid gap-3 sm:grid-cols-[80px_1fr_100px_80px_auto]">
      ...
      <div className="glass-card overflow-hidden p-0">
        <table className="data-table">
          <thead><tr><th>ID</th><th>Flag</th><th>Code</th><th>Name</th><th>Dial</th><th>Enabled</th><th></th></tr></thead>
          <tbody>
            {list.map((c) => (
              <tr key={c.id}>
            ...
            {list.length === 0 && (<tr><td colSpan={7} className="py-10 text-center text-muted-foreground">No countries yet.</td></tr>)}
          </tbody>
        </table>
      </div>
    </>
